...
def save(x,layers,speckle,composite):
    filename1 = sys.argv[2]+"/SVImage/SVImage%03d.tif" % (x + 1)
    filename2 = sys.argv[2]+"/CompositeImage/CompositeImage%03d.tif" % (x + 1)
    np.savetxt(sys.argv[2]+"/Data_CSV/Data%04d.csv" % (x + 1), layers, delimiter=' ', fmt="%d")
    tifffile.imsave(filename1, speckle)
    tifffile.imsave(filename2, composite)
...
def RegFFT(volume_log):
    ...
    for num in range(repeatFrame):
        ...
        a = registration.phase_cross_correlation(frame1, frame2, upsample_factor=usfac, return_error=False)
...
def avgSV(bmFiles):
    ...
    gFilter= cv2.ximgproc.guidedFilter(convertedImage, convertedImage, 1, 3)
    Aline_max = np.max(gFilter)
    div = 255 / Aline_max
    gFilterConverted = gFilter * div
    gFilterConverted = np.uint8(gFilterConverted)
    return gFilterConverted
...
def corrAxialMotion(absLogImage1):
        ...
        motionA = maxxcorrAx(absLogImage1)
...
if __name__ == '__main__':
    ...
    # path = glob.glob('./BM-OCTReconstructedImage/*.tif')
    t.tic()
    # Reading an image
    data = io.loadmat(sys.argv[1]+'/finalStack.mat')
    img = data["finalStack"]
    nImage = img.shape[2]
    t.toc()
    t.tic()
    # Absolute value with Log value of an image
    absLogImage = []
    for i in range(0, nImage):
        absImage = np.absolute(img[:,:,i])
        logValue = math.log(20, 10)
        absLogValue = absImage * logValue
        absLogImage.append(absLogValue)
    ...
    for imageStack in range(0,128):
        ...
        Aline_max = 255 / np.max(avgImg)

        Avg_OCT_log = avgImg * Aline_max

        Avg_OCT_display = np.uint8(Avg_OCT_log)
        AvgReg.append(Avg_OCT_display)
    ...
    for i in range(0,128):
        SpeckleImage = SVImage[i]
        AvgRegImage = AvgReg[i]
        ...
        weightedImg.append(cv2.addWeighted(merged_SV, alpha1, merged_Reg, beta1, 0.5))

    index = np.arange(0, 128)
    Data = zip(index, final_layers, SVImage, weightedImg)
    Data = list(Data)
    savingData = multiprocessing.Pool(processes=5).starmap(save, Data)
    multiprocessing.Pool(processes=5).close()
    t.toc()

    print('RegImage ', np.array(RegImage).shape)
    print('SVImage ', np.array(SVImage).shape)
    print('AvgReg ', np.array(AvgReg).shape)
    print('final_layers ', np.array(final_layers).shape)
    print('weightedImg ', np.array(weightedImg).shape)